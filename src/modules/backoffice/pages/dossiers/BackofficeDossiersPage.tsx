...
import { isAutoAffiliationRequest } from '../../../workspace/workspaceUtils';
...
function isAutoAffiliationToReview(request: AffiliationRequestItem): boolean {
  return isAutoAffiliationRequest(request) && request.status === 'EN_ATTENTE_VALIDATION_BOA';
}
...
export default function BackofficeDossiersPage() {
  ...
  useEffect(() => {
    ...
    getAffiliationRequests()
      .then((response) => {
        ...
        setRequests(
          response.requests
            .filter(isAutoAffiliationToReview)
            .sort((left, right) => right.dossierId - left.dossierId)
        );
        setErrorMessage('');
      })
      .catch(() => {
        if (mounted) setErrorMessage('Impossible de charger les dossiers auto-affiliation.');
      })
      .finally(() => {
  ...
  return (
    <div className="commercial-requests-page">
      <section className="prospection-stats" aria-label="Résumé des dossiers">
      ...
      <section className="filter-card" aria-labelledby="dossiers-filters-title">
        <div className="filter-head">
          <div>
            <span className="filter-head__icon material-icons" aria-hidden="true">tune</span>
            <div>
              <strong id="dossiers-filters-title">Rechercher et filtrer</strong>
      ...
      <section className="table-card" aria-labelledby="dossiers-table-title">
        {errorMessage && <div className="page-alert error">{errorMessage}</div>}
        {isLoading && (
          <div className="loading-state">
            <span className="page-loading-spinner" />
            <span>Chargement des dossiers auto-affiliation...</span>
          </div>
        )}
        {!isLoading && !filteredRequests.length && (
          <div className="empty-state">Aucun dossier auto-affiliation ne correspond aux filtres.</div>
        )}
        {!isLoading && filteredRequests.length > 0 && (
          <div className="table-wrap">
            <div className="table-summary">
              <div>
                <strong id="dossiers-table-title">Dossiers à contrôler</strong>
